<template>
  <div class="hall">
    <div class="hall__row" v-for="(row, row_i) in allPlaces.slice().reverse()" :key="allPlaces.length - row_i - 1">
      ...
      <div class="hall__place" :class="{'hall__place--disabled': item == 1}, {'hall__place--selected': item == 2}"
           v-for="(item, item_i) in row" :key="item_i" @click="pickPlace(allPlaces.length - row_i - 1,  item_i)">
        {{ item_i + 1 }}
      </div>
    </div>
    ...
    <div class="movie__tickets" v-if="tickets">
      ...
      <div class="movie__tickets-item" v-for="(ticket, ticket_i) in tickets" :key="ticket_i"><b>Ряд:</b>
        {{ ticket.row + 1 }} <b>Место:</b> {{ ticket.place + 1 }}
      </div>
    </div>
  </div>
</template>
...
export default {
    ...
    allPlaces() {
      ...
      nonpaidTicketsOnThisMovie.forEach((nonpaidticket) => {
        nonpaidticket.places.forEach((pickedPlace) => {
          ...
          this.tickets.push({row: pickedPlace.row, place: pickedPlace.place});
        });
  ...
  mounted() {
    if (this.$route.query.places) {
      let arrayPlaces = this.$route.query.places.split(';');
      arrayPlaces.splice(arrayPlaces.length - 1, 1);
      arrayPlaces.forEach((ticket) => {
        let rowPlace = ticket.split('-');
        this.tickets.push({row: +rowPlace[0], place: +rowPlace[1]})
        this.hallMap[rowPlace[0]][rowPlace[1]] = 2;
      })
    }
  }
}
