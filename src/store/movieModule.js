export const movieModule = {
    state: () => ({
        movies: [
            ...
            { id: 3, name: 'Шрэк Трейтий', img: 'shrek-3', year: '2007', genre: "cartoon", rate: 4, desc: 'Король Гарольд внезапно умер, и теперь великан Шрек вынужден стать королем Далекой-Далекой страны.'},
    ...
    getters: {
        ...
        SORTED_MOVIES(state) {
            return [...state.movies].sort((movie1, movie2) => new Intl.Collator().compare(movie1[state.selectedSort],movie2[state.selectedSort]))
        },
    },
    mutations: {
